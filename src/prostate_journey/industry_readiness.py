...
def validate_frontend_source(project_root: str | Path) -> CheckResult:
    ...
    root = Path(project_root).resolve()
    sources = [
        root / "src/prostate_journey/executive_dashboard.py",
        root / "src/prostate_journey/frontend/analytics.css",
        root / "src/prostate_journey/frontend/analytics.js",
    ]
...
def validate_presentation_package(
    ...
    html_failures = 0
    executive_assets = ""
    for asset_name in ("assets/analytics.css", "assets/analytics.js"):
        asset_path = root / asset_name
        if not asset_path.is_file():
            html_failures += 1
            continue
        executive_assets += asset_path.read_text(encoding="utf-8")
    ...
    for name in frontend_names:
        ...
        if name == "executive_story.html":
            ...
            html_failures += int(":focus-visible" not in executive_assets)
            html_failures += int('role="dialog"' not in text)
            html_failures += int("Escape" not in executive_assets)
