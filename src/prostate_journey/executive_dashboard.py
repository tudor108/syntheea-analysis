...
<strong>Filters:</strong> all configured scenarios; 90-day initiation; 60-day persistence gap.<br>
<strong>Method:</strong> certified aggregate cohort, landmark, and transition summaries.</p>
...
<text x="100" y="700" font-family="Arial" font-size="17" fill="#526579">Release: {html.escape(str(provenance["release_id"]))} · source: {html.escape(str(provenance["source_commit"]))}</text>
<text x="100" y="730" font-family="Arial" font-size="17" fill="#526579">State: all configured scenarios · initiation 90 days · persistence gap 60 days · primary KPI {segment["initiated90"]:,}/{segment["eligible"]:,}</text>
<text x="100" y="760" font-family="Arial" font-size="17" fill="#526579">Method: censor-aware aggregate landmark summary · generated: {html.escape(str(provenance["generated_at"]))}</text>
<text x="100" y="810" font-family="Arial" font-size="17" fill="#526579">{html.escape(PROHIBITED_USE)}</text></svg>"""
...
<p><strong>Release:</strong> <code>{html.escape(str(provenance["release_id"]))}</code><br>
<strong>Source commit:</strong> <code>{html.escape(str(provenance["source_commit"]))}</code><br>
<strong>Configuration hash:</strong> <code>{html.escape(str(provenance["configuration_hash"]))}</code><br>
<strong>Generated:</strong> {html.escape(str(provenance["generated_at"]))}<br>
<strong>State:</strong> all configured scenarios; 90-day initiation; 60-day persistence gap.<br>
<strong>Primary result:</strong> {segment["initiated90"]:,}/{segment["eligible"]:,} eligible synthetic records initiated by day 90.<br>
<strong>Method:</strong> censor-aware aggregate landmark summary.</p>
<p>Use these fixed boards if live interaction is unavailable.</p>
