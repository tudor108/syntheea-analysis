...
def _settings(config: Path, seed: Optional[int], cohort_size: Optional[int], scenario_version: Optional[str], log_level: str):
    configure_logging(log_level)
    return load_config(config, random_seed=seed, target_cohort_size=cohort_size, scenario_version=scenario_version)


@app.command()
def generate(
    config: Path = typer.Option(ROOT / "configs/prostate_scenario.yaml"), seed: Optional[int] = None,
    cohort_size: Optional[int] = None, input_dir: Path = typer.Option(ROOT / "data/raw/synthea"),
    output_dir: Path = typer.Option(ROOT / "data/gold"), scenario_version: Optional[str] = None,
    log_level: str = "INFO",
) -> None:
...
def validate(output_dir: Path = typer.Option(ROOT / "data/gold"), log_level: str = "INFO") -> None:
    ...
    configure_logging(log_level); results = validate_tables(load_exported_tables(output_dir))
    write_quality_report(results, ROOT / "data/reports"); raise_on_critical(results)
