...
- completed-calendar-year age (including birthday/leap-day boundaries), Gleason/ISUP,
  metastatic/hormonal coherence, and eligibility reconstruction/reasons;
- provider specialty/setting consistency, distinct referral endpoints, referral completion chronology,
  a real destination-provider event for every completed referral, and treatment-specialty compatibility;
- regimen/component containment, full-episode intensification reconstruction, leakage-safe
  treatment-start snapshots, and prescription sequence/coverage/summary agreement;
- true ARPI replacement, switch/restart non-overlap, no old-line service or coverage after
  switch, and AS transition logic;
...
- mart reconciliation for eligibility, censor-aware 30/60/90-day initiation, outcomes, and
  care setting.
...
The final-release gate reloads the frozen CSV/Parquet/DuckDB artifacts and independently rebuilds
eligibility, first treatment, initiation windows, regimen/intensification, persistence,
discontinuation, switch/restart, active surveillance, referral transition, and censoring/outcomes.
All ten concepts require zero row-level mismatches.
