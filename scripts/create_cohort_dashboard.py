...
def chart(title: str, labels: list[str], values: list[float], percent: bool = False) -> str:
    ...
    for index, (label, value) in enumerate(zip(labels, values, strict=True)):
        ...
        out.append(
            f'<text x="0" y="{y + 15}" class="axis-label">{html.escape(label)}</text><rect x="{left}" y="{y}" width="{bar_width:.1f}" height="22" rx="8" fill="{color}"/><text x="{left + bar_width + 8:.1f}" y="{y + 16}" class="value-label">{text}</text>'
        )
...
def build_dashboard(rows: list[dict[str, str]]) -> str:
    ...
    eligible_setting = [
        sum(as_bool(row["eligible_for_arpi"]) for row in settings[name]) for name in setting_names
    ]
    gap_rates = [
        100
        * sum(as_bool(row["eligible_not_initiated_90d"]) for row in settings[name])
        / max(1, eligible_setting[index])
        for index, name in enumerate(setting_names)
    ]
    cards = [
        ("Total cohort", total, "patients"),
        ("mHSPC", mhspc, "synthetic"),
        ("Eligible ARPI", eligible, "demo rule"),
        ("Initiated ≤90d", initiated90, "patients"),
        ("Treatment gap", gap, "eligible not started"),
        ("Persistent 12m", persistent12, "patients"),
    ]
    card_html = "".join(
    ...
    segment_rows = "".join(
        f"<tr><td>{html.escape(setting)}</td><td>{age}</td><td>{eligible_count:,}</td><td>{gap_count:,}</td><td>{100 * gap_count / max(1, eligible_count):.1f}%</td></tr>"
        for (setting, age), (eligible_count, gap_count) in sorted(
            segments.items(), key=lambda item: item[1][1] / max(1, item[1][0]), reverse=True
        )[:12]
    )
    sections = [
        chart(
            ...
            [mhspc, eligible, initiated90, persistent12],
        ),
        chart("Prostate stage", stage_labels, stage_values),
        chart("Eligible patients by care setting", setting_names, eligible_setting),
        chart("Treatment gap rate by care setting", setting_names, gap_rates, True),
        chart(
            "Persistence landmarks",
            ["3 months", "6 months", "12 months"],
            [
                sum(as_bool(r["persistent_3m"]) for r in rows),
                sum(as_bool(r["persistent_6m"]) for r in rows),
                persistent12,
            ],
        ),
        chart("Final outcomes", outcome_labels, outcome_values),
        chart("Initial treatment", treatment_labels, treatment_values),
        f'<div class="table-wrap"><h3>Largest treatment gaps by segment</h3><table><thead><tr><th>Care setting</th><th>Age group</th><th>Eligible</th><th>Gap</th><th>Gap rate</th></tr></thead><tbody>{segment_rows}</tbody></table></div>',
    ]
